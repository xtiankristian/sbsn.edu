						...
						<form action="editinfo.php" method="POST"/>
							<div>
								...
								if($_SERVER['REQUEST_METHOD'] == 'POST') { //if the register was clicked
									...
									$user2 = "$user"."_sbsn";
									if(empty($password)){
									$user_input = mysql_query("UPDATE $user2 SET section='$section', address='$address', zip_code='$zip', phone_number='$pnumber', cell_number='$cnumber', fname='$fname', mname='$mname', lname='$lname', email='$email' WHERE username='$user'");
									echo("<p>Succesfully Edited!</p>");
									}
									else{
									$query = mysql_query("Select password from facukty WHERE username='$user'");
									while($row = mysql_fetch_assoc($query)){
										...
										if($pass != $password){
											...
											mysql_query("UPDATE faculty SET password='$npassword' WHERE username='$user'");
...
?>
									...
									$q = mysql_query("SELECT * from faculty WHERE subject = '$subject'");
									$usertable = "";
									while($r = mysql_fetch_assoc($q)){
										$user = $r['username'];
									}
								
									$usertable = "$user" . "_sbsn";
									...
									?>
									<tr>
										<td><a href="adfacultyinfo.php"><label for="name"><span>C</span>ancel Edit</label></td>
									</tr>		
									<tr>
										<td><label for="name"><span>U</span>sername:</label></td>
										<td><label for="name"><span><?php Print"$user"?></label></td>
									</tr>
									<tr>
